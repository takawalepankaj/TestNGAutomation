...
// Close the file handle
     ...
     * @param fileList
     */
    public void getAllFiles(File dir, java.util.List<File> fileList) {
        try {
            log.debug("Entering into Method : " + Thread.currentThread().getStackTrace()[1].getMethodName());
     ...
     * @param sZipFileName
     */
    public void writeZipFile(File directoryToZip, java.util.List<File> fileList, String sZipFileName) {
...
// to the directory being zipped, so chop off the rest of the path
    ...
    public String takeDesktopScreenShot(String sSnapFolderName, String sSnapName) {
        ...
        try{
            ...
            java.awt.Rectangle captureSize = new java.awt.Rectangle(Toolkit.getDefaultToolkit().getScreenSize());
...
//Get Column Header details
                    if ( bHeader ){
                        ...
                        java.util.List<WebElement> cells = row.findElements(By.tagName("th"));
                        if (cells.size() == 0){
    ...
    public boolean selectTableRow(WebElement table, String sData, boolean bCheckNode, boolean bHeaderExits){
        try{
            ...
            if (tableRow != null){
                java.util.List<WebElement> inputList = tableRow.findElements(By.tagName("input"));
...
// }
    ...
    public boolean clickTableCell(WebElement table, String sData, boolean bHeaderExits){
        try{
            ...
            if (tableRow != null){
                boolean bFlag= false;
                java.util.List<WebElement> inputList = tableRow.findElements(By.tagName("a"));
...
//selectedOption.selectByVisibleText(sItem);
     ...
     */
    public java.util.List<String> getDropdownValues(WebDriver driver, WebElement element) {
        log.debug("Entering into Method : " + Thread.currentThread().getStackTrace()[1].getMethodName());

        java.util.List<String> lstOptions = new ArrayList<String>();
        try{
            Select selectedOption = new Select(element);

            java.util.List<WebElement> lstOpt = selectedOption.getOptions();
...
//Get Suite Browser Name
     ...
     *
     */
    public java.util.List<String> getContextMenuOptions(WebDriver driver, String sClassName) {
        java.util.List<String> lsOptions = new ArrayList<String>();
// Preserve list of context menu options web elements
        java.util.List<WebElement> Optionset = driver.findElements(By.className(sClassName));
